...
# copied from TYPO3 core v 10.4.15-dev
...
usage()
{
    ...
    echo "      - all: check all files in Classes                  "
    echo " "
    echo " -n                                                      "
...
if [[ $filestype == commit ]];then
    ...
    DETECTED_FILES=$(cat)
elif [[ $filestype == all ]];then
    echo "$progname: reading list of php files to check from Classes"
    DETECTED_FILES=$(find ${phpdirs} -name "*.php")
else
...
# ---------------------------------
exist_php_executable
php_no_xdebug ${bindir}/php-cs-fixer fix \
    -v ${DRYRUN} \
